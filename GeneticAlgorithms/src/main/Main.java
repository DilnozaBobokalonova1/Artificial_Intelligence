...
public class Main {
	static void initMap(Map map) { //initializes a map with 10 states and borders between the states
	...
	public static void main(String[] args) {
			Map map = new Map();
			initMap(map); //fills up the map with states and borders
			
			final int populationSize = 50; // TODO find reasonable value
			Population population = new Population(map, populationSize);

			final int maxIterations = 2000; // TODO find reasonable value
			...
			while(currentIteration < maxIterations && !goalFound) {
				...
				for(int i = 0; i < populationSize; ++i) {
					...
					if(Math.random() > 0.01) // TODO use small probability instead
					{
						child.mutate();
					}
					if(child.isGoal())
			...
			if(goalFound) {
				...
				bestIndividual.print(); //using the print method found in individual
			}
			else {
